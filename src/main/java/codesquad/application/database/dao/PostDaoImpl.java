...
public class PostDaoImpl implements PostDao {
    ...
    public long save(PostVO data) {
        String sql = "INSERT INTO posts (post_id, user_id, content, image_path, created_at) VALUES (%s, %s, %s);";
        sql = String.format(sql, data.userId(), data.content(), data.imagePath());
...
//            pstmt.setString(3, data.imagePath());
    ...
    public Optional<PostVO> findById(long id) {
        String sql = "SELECT post_id, user_id, content, image_path, created_at FROM posts WHERE post_id = " + id + ";";
        try (Connection conn = databaseConfig.getConnection();
    ...
    public Collection<PostVO> findAll() {
        String sql = "SELECT post_id, user_id, content, image_path, created_at FROM posts;";
